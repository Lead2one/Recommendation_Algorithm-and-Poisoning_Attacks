...
def targetItemSelect(data, arg, popularThreshold=0.1):
    ...
    if os.path.exists(path):
        ...
        if arg.attackTargetChooseWay == "random":
            targetItem = random.sample(list(set(list(range(itemNum)))),
                                       targetNum)
        elif arg.attackTargetChooseWay == "popular":
            targetItem = random.sample(list(set(getPopularItemId(int(popularThreshold * itemNum)))),
                                       targetNum)
        elif arg.attackTargetChooseWay == "unpopular":
            targetItem = random.sample(
                list(set(getReversePopularItemId(int(0.2 * itemNum)))),
                targetNum)
            # targetItem = random.sample(

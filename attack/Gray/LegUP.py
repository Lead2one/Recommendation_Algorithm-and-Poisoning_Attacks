...
class LegUP():
    ...
    def posionDataAttack(self, epoch1=25, epoch2=25):
        ...
        """
        if self.G is None:
            self.selectItem = random.sample(list(set(list(range(self.itemNum)))-set(self.targetItem)), self.itemNum//5)  + self.targetItem
            ...
            for i in range(self.BiLevelOptimizationEpoch):
                ...
                for k1 in range(epoch1):
                    userSet = random.sample(list(range(self.userNum)),
                                            self.fakeUserNum)
                    # #sample item rating

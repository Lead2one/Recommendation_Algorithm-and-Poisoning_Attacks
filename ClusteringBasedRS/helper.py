...
def get_most_rated_movies(user_movie_ratings, max_number_of_movies):
    # 1- Count
    count_row = pd.DataFrame(user_movie_ratings.count()).transpose()
    user_movie_ratings_with_count = pd.concat([user_movie_ratings, count_row], ignore_index=True)
    
    # 2- sort
    user_movie_ratings_sorted = user_movie_ratings_with_count.sort_values(by=user_movie_ratings_with_count.index[-1], axis=1, ascending=False)

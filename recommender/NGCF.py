...
class NGCF():
    def __init__(self, args, data):
        ...
        # self.model = LGCN_Encoder(self.data, args.emb_size)
        self.model = NGCF_Encoder(self.data, self.args.emb_size, self.args.n_layers)
    ...
    def forward(self):
        ...
        for k in range(self.layers):
            ...
            ego_embeddings = F.leaky_relu(torch.sparse.mm(self.sparse_norm_adj, temp) + \
                                          temp + \
                                          torch.mm(
                                              torch.sparse.mm(self.sparse_norm_adj, ego_embeddings) * ego_embeddings,
                                              self.W['w2_' + str(k)]))
            all_embeddings += [ego_embeddings]

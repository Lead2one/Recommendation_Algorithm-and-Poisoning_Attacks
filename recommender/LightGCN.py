...
class LightGCN():
    def __init__(self, args, data):
        ...
        # emit('training_update', {'chars': "Recommender: LightGCN"})
        self.data = data
        self.args = args
        ...
        for epoch in range(maxEpoch):
            for n, batch in enumerate(next_batch_pairwise(self.data, self.args.batch_size)):
                ...
                if n % 1000 == 0:
                    ...
                    emit('training_update', {'epoch': epoch + 1, 'loss': batch_loss.item()})
            model.eval()
            with torch.no_grad():
    ...
    def evaluate(self, epoch):
        ...
        # emit('training_update', {'chars': 'Evaluating the model...'})
        ...
        if len(self.bestPerformance) > 0:
            ...
            self.save()
        print('-' * 120)
        # emit('training_update', {'chars': '-' * 120})
        print('Real-Time Ranking Performance ' + ' (Top-' + str(self.max_N) + ' Item Recommendation)')
        # emit('training_update', {'chars': 'Real-Time Ranking Performance ' + ' (Top-' + str(self.max_N) + ' Item Recommendation)'})
        measure = [m.strip() for m in measure[1:]]
        print('*Current Performance*')
        # emit('training_update', {'chars': '*Current Performance*'})
        print('Epoch:', str(epoch + 1) + ',', '  |  '.join(measure))
        # emit('training_update', {'chars': 'Epoch:' + str(epoch + 1) + ',' + '  |  '.join(measure)})
        bp = ''
        # for k in self.bestPerformance[1]:
        ...
        # emit('training_update', {'chars': '*Best Performance* '})
        print('Epoch:', str(self.bestPerformance[0]) + ',', bp)
        # emit('training_update', {'chars': 'Epoch:' + str(self.bestPerformance[0]) + ',' + bp})
        print('-' * 120)
        # emit('training_update', {'chars': '-' * 120})
        return measure
